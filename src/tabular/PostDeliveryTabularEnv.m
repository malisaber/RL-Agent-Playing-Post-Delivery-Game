classdef PostDeliveryTabularEnv < handle
	properties
		max_length;
		Action_length; 
		Dynamics;
	...
	methods
		% Constructor
		function obj = PostDeliveryTabularEnv(varargin)
			if nargin == 8
				obj.max_length = varargin{1};
				obj.Action_length = varargin{2};
				obj.Dynamics = varargin{3};
				obj.Rewards = varargin{4};
				obj.pack_poses = varargin{5};
				obj.c2s_f = varargin{6};
				obj.s2c_f = varargin{7};
				obj.TC_f  = varargin{8};
			elseif nargin == 7
				obj.max_length = 0;
				obj.Action_length = varargin{1};
				obj.Dynamics = varargin{2};
				obj.Rewards = varargin{3};
				obj.pack_poses = varargin{4};
				obj.c2s_f = varargin{5};
				obj.s2c_f = varargin{6};
				obj.TC_f  = varargin{7};
			else
				error("PostDeliveryTabularEnv expects 7 or 8 inputs.");
			end
			obj.car_pos.x = 0;
			obj.car_pos.y = 0;
			obj.pac_pos.x = 0;
		...
		% generate an episode
		function [episode_states, episode_actions, episode_rewards] = generate_episode(obj, policy)
			ep_all_states  = zeros(1, obj.max_length+1);
			ep_all_actions = zeros(1, obj.max_length+1);
			ep_all_rewards = zeros(1, obj.max_length+1);
			
			this_car_pos = obj.car_pos;
			
			for ep_cntr = 1:obj.max_length
				this_state  = obj.c2s_f(obj.which_package-1, this_car_pos.y, this_car_pos.x);
				...
				this_car_pos.x = next_pos(3);
				this_car_pos.y = next_pos(2);
				...
				if obj.TC_f(this_car_pos.x, this_car_pos.y,...
		...
		% simulate an episode
		function [Aeps, Aepa, Aepr] = simulate(obj, runs, policy, sleep_time)
			obj.show();
			Aeps = cell(runs, 1);
			Aepa = cell(runs, 1);
			Aepr = cell(runs, 1);
		...
		function Plot_gridworld_actions(obj, this_Policy, name)
			figure;
			t = tiledlayout(2,2);
