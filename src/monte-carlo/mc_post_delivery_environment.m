classdef mc_post_delivery_environment < handle
	properties
		max_length;
		Action_length; 
		Dynamics;
	...
	methods
		% Constructor
		function obj = mc_post_delivery_environment(max_length, action_length, ...
				dynamics, rewards, goal_pos, Funcs)
			obj.max_length = max_length;
			obj.Action_length = action_length;
			obj.Dynamics = dynamics;
			obj.Rewards = rewards;
			obj.pack_poses = goal_pos;
			obj.c2s_f = Funcs.conv_cordn2state;
			obj.s2c_f = Funcs.conv_state2cordn;
			obj.TC_f  = Funcs.is_terminal_cond;
			obj.car_pos.x = 0;
			obj.car_pos.y = 0;
			obj.pac_pos.x = 0;
			obj.pac_pos.y = 0;
		...
		% simulate an episode
		function [Aeps, Aepa, Aepr] = simulate(obj, runs, name, policy, sleep_time, path)
			obj.show();
			Aeps = cell(runs, 1);
			Aepa = cell(runs, 1);
			Aepr = cell(runs, 1);
		...
		function Plot_actions_base(~, this_Policy, ax)
			hold(ax, "on")
			axis(ax, "off")
			axis(ax, "equal")
			m = 5;
		...
		function Plot_state_value_base(~, this_Q_val, color_bar_range, ax)
			hold(ax, "on")
			axis(ax, "off")
			axis(ax, "equal")
			m = 5;
