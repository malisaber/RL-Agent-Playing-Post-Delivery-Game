classdef Network < handle
	...
	methods
		%	Constructor
		function obj = Network(inp_size, hid_size, out_size, alpha)
		...
		%	Plot the graph
		function plot_net(obj, path)
			this_fig = figure;
			plot(obj.Lgraph)
			exportgraphics(this_fig, fullfile(path, "net.jpg"), 'Resolution', 600);
		end
		...
		function loss = Train(obj, input, targets, actions, iter)
			...
			[loss, gradients] = dlfeval(@Calc_grad, obj.Net, input, targets, actions);
